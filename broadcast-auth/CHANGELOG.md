...
### Fixed
- **`Verifier::verify`'s Digest check wrongly rejected the absolute-form
  request-target (RFC 7616/RFC 7230 §5.3.2)** — issue #724. Two related
  faults in `verify_digest`:
  1. `HA2` was computed as `MD5(method:request_uri)` using the *server's*
     request URI, not the client's own claimed `uri` field (the
     `digest-uri-value` RFC 7616 §3.4.1 actually defines `HA2` over). A
     client whose challenge round-trip legitimately hashed a
     different-but-equivalent request-target (e.g. the absolute-form URL)
     could never produce a `response` this recomputation would match, even
     with the correct password.
  2. The RFC 7616 §3.4.1 SHOULD uri-match was a strict `get("uri") !=
     request_uri` reject, which rejected the absolute-form representation
     outright rather than recognising it as the same target.
  ...
  `uri` — a form RFC 7616 and this crate's own `RequestContext` docs already
  permitted, but that `Verifier::verify` couldn't actually accept. `HA2` now
  uses the client's own `uri` field; the SHOULD-check is now
  [`digest_uri_matches`] (new, crate-internal), which accepts either
  origin-form (`/path[?query]`) or absolute-form
  (`scheme://authority/path[?query]`) whose path(+query) is byte-identical
  to the server's request URI — a genuinely different `uri` (in either form)
  is still rejected exactly as before, and the constant-time response
