...
//!
//! **Scope of this release**: the Caller-Listener flow only (§4.3.1,
//! induction → conclusion). Rendezvous (§4.3.2, WAVEAHAND/AGREEMENT) is not
//! implemented. TSBPD delivery, ARQ/loss, congestion control, actual AES
//! key-wrap/unwrap crypto, and a `tokio` socket adapter are explicit
//! follow-ups — see the crate root docs.
...
pub(crate) struct ParsedPeerExtensions {
...
pub(crate) fn parse_peer_extensions(hp: &HandshakePacket<'_>) -> Result<ParsedPeerExtensions> {
