//! Drift-guard: every `pub enum` in `atsc3-route` must carry
//! `#[non_exhaustive]` (issue #806).
//!
//! Scans `src/` for every `pub enum`, subtracts a documented SKIP list, and
//! fails if any remaining enum lacks a `#[non_exhaustive]` attribute on one
//! of the five lines immediately preceding the `pub enum` declaration.
//!
//! SKIP is intentionally empty: every public enum in this crate (including
//! `Error` and the dispatch enum `RouteFecPayloadId`) is already
//! non-exhaustive.
