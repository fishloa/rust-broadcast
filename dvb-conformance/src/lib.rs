...
struct CcState {
    ...
    /// with [`broadcast_common::ts_dup::check_duplicate`] to test the
    /// H.222.0 §2.4.3.3 "legal duplicate" byte-identity rule (that function
    /// owns locating and exempting the PCR field). Empty until the first
    /// packet is recorded.
    last_packet: Vec<u8>,
}
    ...
    fn check_cc(&mut self, pid: u16, cc: u8, has_payload: bool, t: Duration, raw: &[u8]) {
        ...
        let (expected, is_duplicate, should_emit_dup, should_emit_cc) = {
            ...
            if discontinuity {
                ...
                // ISO/IEC 13818-1:2023 §2.4.3.3: a legal duplicate requires
                // the same continuity_counter on a payload-bearing packet
                // AND byte-identical content (PCR excepted), AND at most
                // one such repeat in a row ("two, and only two"). Comparing
                // only `cc == state.last_cc` (as this code used to) would
                // accept ANY packet repeating the previous CC, silently
                // swallowing genuine continuity faults (issue #956).
                use broadcast_common::ts_dup::{DuplicateVerdict, check_duplicate};

                let verdict = check_duplicate(&state.last_packet, raw, state.dup_used);
                let is_duplicate = !matches!(verdict, DuplicateVerdict::NotDuplicate);
        ...
        if discontinuity {
            ...
            // A duplicate (legal or an illegal third-plus repeat) is, by
            // construction, byte-identical (PCR excepted) to what's already
            // stored, so `last_cc`/`last_packet` are left in place — only
            // `dup_used` moves, so a further repeat is still recognised as
