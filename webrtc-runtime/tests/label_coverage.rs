...
//!
//! webrtc-runtime's public enums are state-machine types (State, Event,
//! Method) and a structured error — all skipped from the label convention.
//! This test still asserts the known set so new spec/field enums get caught.

use std::collections::BTreeSet;
use std::fs;
use std::path::Path;

const SKIP: &[&str] = &["Error", "Event", "Method", "State"];
