...
- **Version table** — every crate's `Cargo.toml` `version`. The 7 lockstep core crates (`dvb-common`, `dvb-si`, `dvb-t2mi`, `dvb-bbframe`, `dvb-scte35`, `dvb-conformance`, `dvb-tools`) must be identical. Independents (`dvb-stream`, `dvb-pes`, `dvb-subtitle`, `mpeg-ps`, `scte104`, `dvb-cc`, `dvb-ci`) version on their own cadence.
