...
- **Version table** — every crate's `Cargo.toml` `version`. The 6 lockstep core crates (`dvb-common`, `dvb-si`, `dvb-t2mi`, `dvb-bbframe`, `dvb-conformance`, `dvb-tools`) must be identical. Independents (`dvb-stream`, `scte35-splice`, `mp4-emsg`, `mpeg-pes`, `dvb-subtitle`, `mpeg-ps`, `scte104`, `cc-data`, `smpte2038`, `ule`, `dvb-ci`) version on their own cadence.
