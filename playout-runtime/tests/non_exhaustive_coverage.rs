//! Drift-guard: every `pub enum` in playout-runtime must carry
//! `#[non_exhaustive]` (issue #806 convention, copied from
//! `webrtc-runtime/tests/non_exhaustive_coverage.rs`).
...
fn every_public_enum_is_non_exhaustive() {
    ...
    assert!(
        ...
        "pub enum(s) missing `#[non_exhaustive]`: {missing:?}\n\
         ...
         or add the enum to SKIP with a reason."
    );
}
