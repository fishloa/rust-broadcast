...
fuzz_target!(|data: &[u8]| {
    ...
    // RepairFecPayloadId, and RoutePacket all implement Parse/Serialize.
    // Fuzz each with byte-exact round-trip validation — must not panic on
    // malformed input, and a successful parse must round-trip
    // byte-identically.
    ...
    // Test 3: RoutePacket (composed ALC/LCT packet, §A.3.4/§A.3.6)
