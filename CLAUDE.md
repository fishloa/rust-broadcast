...
- **multimux** — multi-input (RTSP/RTP/TS-UDP/TS-HTTP/HLS-pull) × multi-output (LL-HLS/DASH/LL-DASH) just-in-time repackaging HTTP origin (library: tokio + axum): shared output auth (Basic/Digest/Bearer/Forwarded) across every route, an external scheme plugin registry (`Custom` input/output/output-auth + `SchemeRegistry`) so a third-party crate can add a scheme without editing this crate, Prometheus metrics + health/readiness, supervised reconnect. Built on `rtsp-runtime`/`ll-hls-runtime`/`broadcast-auth`/`transmux`. Independently versioned.
