...
- **broadcast-loudness** — EBU R 128 / ITU-R BS.1770-5 loudness measurement: K-weighting filter, gated integrated loudness (LUFS), momentary/short-term, loudness range (EBU Tech 3342) and true peak (dBTP). **48 kHz only** — BS.1770-5 Annex 1 tabulates its coefficients for 48 kHz and gives no derivation for other rates, so `LoudnessMeter::new` returns `Error::UnsupportedSampleRate` rather than silently mismeasuring; non-finite input samples are rejected for the same reason (one NaN would poison the IIR state permanently). Verified against the EBU Tech 3341 compliance test signals. No wire format, so the workspace round-trip invariant does not apply — the compliance vectors are its equivalent hard gate. `no_std`+`alloc`, depends only on broadcast-common; independently versioned.
