...
the standalone `multimux-cli` server uses), fed by a `VdoIngestSession` instead
of RTSP.
...
- **`src/vdo_source.rs`** (`device`) — drives `vdo::StreamBuilder` /
  ...
  directly (no `Dialer`: VDO is a local capture, not a dial-out — see the
  module's own doc). Driven by a plain `IngestDriver` the caller (the bin)
  builds itself, not `multimux::pipeline::run_pipeline` (deleted in multimux
  0.5, issue #805).
...
- **`src/bin/acap-multimux.rs`** (`device`) — the ACAP entrypoint: loads config,
  ...
  via `multimux::supervise_driver`/`multimux::source::advance_route`, serves
  the origin + admin on `127.0.0.1:2999`.
...
`rust-toolchain.toml`). It depends on published `multimux` 0.5 + `transmux` 0.20
+ `media-plane` 0.1 + `broadcast-common` 9 (crates.io, the latter two only
under the `device` feature — see `vdo_source.rs`) and **git-pinned** acap-rs
crates (`vdo`/`axparameter`/`acap-logging` at rev `e9a838d` via a fork
`fishloa/acap-rs`). It is **not** published to crates.io — it is a deployable
...
> `VdoIngestSession`'s `data_copy()` handling.
