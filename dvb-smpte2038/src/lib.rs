//! SMPTE ST 2038:2021 — carriage of ANC (SMPTE ST 291-1 ancillary) data
//! packets in an MPEG-2 Transport Stream.
//!
//! ST 2038 provides a transparent pipe so ST 291-1 ANC data packets can be
//! conveyed frame-accurately alongside the video they belong to (ST 2038 §1).
//! It is **not** for audio carriage and **not** for EDH packets (Introduction).
//!
//! This crate implements the two wire structures defined in §4:
//!
//! - [`AncDataDescriptor`] — the `anc_data_descriptor` (tag `0xC4`) used in the
//!   PMT ES loop, plus the `"VANC"` `registration_descriptor`
...
#![doc = "\n## Runnable examples\n"]
#![doc = "Run with `cargo run -p dvb-smpte2038 --example <name>`.\n"]
#![doc = "\n### `build_anc`\n\n```rust,ignore"]
